...
const ProductShowcase = () => {
	...
	return (
		<section className='py-20 bg-gradient-to-b from-primary-900/50 via-primary-50 to-white'>
			<div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
				...
				<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8'>
					{products.map((product, index) => (
						<motion.div
							...
							className='group cursor-pointer bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform group-hover:-translate-y-2 flex flex-col'>
							<Link to={`/products/${product.id}`}>
								<div className='relative aspect-square w-full overflow-hidden'>
									...
									/>
									<div
										className={`absolute inset-0 bg-gradient-to-t ${product.color} opacity-0 group-hover:opacity-20 transition-opacity duration-300 rounded-t-2xl`}
									/>
								</div>
				...
				<motion.div
					...
					<Link
						to='/products'
						className='inline-flex items-center bg-primary-600 hover:bg-primary-700 text-white px-8 py-3 rounded-full font-semibold text-lg transition-all transform hover:scale-105'>
						View All Products
